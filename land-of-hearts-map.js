window.HPC_LAND_OF_HEARTS_MAP = {
  init(businesses) {
    ...
    const map = L.map(mapEl, { scrollWheelZoom: false }).setView([27.90, -81.60], 9);
    ...
    function popupFor(b) {
      const desc = b.public_description || b.description || 'Affirming business listing.';
      const place = [b.address, b.city].filter(Boolean).join(', ');
      return '<div class="loh-popup"><small>' + (b.category || 'Local Business') + '</small><h3>' + b.name + '</h3><p>' + desc + '</p><p>' + place + '</p>' + iconsFor(b) + '</div>';
    }
    ...
    function render(list) {
      ...
      list.forEach((b, index) => {
        ...
        points.push(point);
        L.marker(point, { icon: heart, title: b.name }).addTo(layer).bindPopup(popupFor(b));
      });
