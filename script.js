...
if (navLinks) {
  const navItems = [
    ['about.html', 'About'], ['programs.html', 'Programs'], ['resources.html', 'Resources'], ['crisis.html', 'Crisis'], ['partners.html', 'Partners'], ['volunteer.html', 'Get Involved'], ['support.html', 'Support HPC'], ['contact.html', 'Contact']
  ];
  navItems.forEach(([href, label]) => {
    ...
    if (!exists) {
      ...
      if (href === 'support.html' && contactLink) navLinks.insertBefore(link, contactLink); else navLinks.appendChild(link);
    }
  });
}
...
if (menuToggle && navLinks) {
  ...
  navLinks.querySelectorAll('a').forEach((link) => link.addEventListener('click', () => {
    navLinks.classList.remove('is-open'); document.body.classList.remove('menu-open'); menuToggle.classList.remove('is-open'); menuToggle.setAttribute('aria-expanded', 'false');
  }));
}
...
if (footer) {
  ...
  footer.innerHTML = `<div class="footer-rainbow" aria-hidden="true"></div><div class="footer-grid"><div class="footer-brand"><h2>Heartland Pride Center</h2><p class="footer-tagline">Real people helping real people.</p><p>Building an affirming LGBTQ+ community resource hub for Polk County and Florida's Heartland.</p></div><div><h3>Explore</h3><a href="about.html">About</a><a href="programs.html">Programs & Services</a><a href="resources.html">Resource Directory</a><a href="crisis.html">Crisis & Immediate Support</a><a href="support.html">Support HPC</a></div><div><h3>Get Involved</h3><a href="volunteer.html">Volunteer</a><a href="partners.html">Partner With Us</a><a href="resources.html">Join the Resource Network</a><a href="contact.html">Contact HPC</a><a href="${givebutterUrl}" target="_blank" rel="noopener">Givebutter</a></div><div><h3>Connect</h3><a href="mailto:info@heartlandpridecenter.org">info@heartlandpridecenter.org</a><a href="https://www.facebook.com/people/Heartland-Pride-Center/61577565114125/" target="_blank" rel="noopener">Facebook</a><a href="https://www.instagram.com/heartlandpridecenter/" target="_blank" rel="noopener">Instagram</a><a href="crisis.html">Need immediate support?</a></div></div><div class="footer-bottom"><p>© 2026 Heartland Pride Center, Inc. A Florida nonprofit organization.</p><p>Serving Polk County and the Central Florida Heartland.</p></div>`;
}

const givebutterButtons = document.querySelectorAll('a[href="#givebutter-link-needed"]');
givebutterButtons.forEach((button) => {
  button.href = givebutterUrl; button.target = '_blank'; button.rel = 'noopener'; button.textContent = 'Support Through Givebutter';
...
brandPolish.textContent = `
  .site-header{overflow:visible;padding-top:18px;padding-bottom:18px}.site-header::after{content:"";position:absolute;left:0;right:0;bottom:-4px;height:4px;background:linear-gradient(90deg,#e63946,#f4a261,#e9c46a,#2ecc71,#3a86ff,#9b5de5);box-shadow:0 0 18px rgba(212,175,55,.18)}.brand{overflow:visible}.brand img{width:74px;height:74px;object-fit:contain;object-position:center;overflow:visible}.nav-links{gap:clamp(12px,1.7vw,24px)}.nav-links a{white-space:nowrap}.rainbow-signature-strip,.footer-rainbow{width:min(1180px,88vw);height:5px;margin:0 auto;border-radius:999px;background:linear-gradient(90deg,#e63946,#f4a261,#e9c46a,#2ecc71,#3a86ff,#9b5de5);box-shadow:0 0 28px rgba(212,175,55,.2)}.subtle-link{display:inline-block;margin-top:10px;color:var(--gold-light);font-weight:950;font-size:.82rem;letter-spacing:.08em;text-transform:uppercase}
